...
from kivy_ios.project.staging import create_staging
...
class TestPbxprojGeneration:
    ...
    def test_run_script_guards_uncollected_slice(self, config, project_root):
        ...
        layout = materialize_project(config, project_root)
        text = (layout.xcodeproj / "project.pbxproj").read_text()
        assert "has no collected pip-deps for this platform" in text
    ...
    def test_legacy_pip_deps_reference_self_heals(self, config, project_root):
        # Projects generated by older toolchains carried pip-deps in Copy Bundle
        # Resources; regeneration must strip it so device builds never stage the
        # simulator slice ahead of the run script's rsync.
        from pbxproj.pbxextensions.ProjectFiles import FileOptions, TreeType

        layout = materialize_project(config, project_root)
        pbx_path = layout.xcodeproj / "project.pbxproj"
        (layout.root / "pip-deps").mkdir(exist_ok=True)
        project = XcodeProject.load(str(pbx_path))
        project.add_file(
            "pip-deps",
            target_name="touchtracer",
            tree="SOURCE_ROOT",
            file_options=FileOptions(embed_framework=False),
        )
        project.save()
        assert "pip-deps in Resources" in pbx_path.read_text()

        XcodeProjectGenerator(config, layout).generate()
        assert "pip-deps in Resources" not in pbx_path.read_text()
        project = XcodeProject.load(str(pbx_path))
        # The legacy build reference (path "pip-deps") is gone; the browse-only
        # reference (path "pip-deps-simulator") is unaffected by the path-based
        # self-heal.
        assert not project.get_files_by_path("pip-deps", tree=TreeType.SOURCE_ROOT)
        browse = project.get_files_by_path(
    ...
    def test_ios_platform_shim_written(self, config, project_root):
        layout = materialize_project(config, project_root)
        shim = layout.platform / "ios.py"
        assert shim.is_file()
        text = shim.read_text(encoding="utf-8")
        assert "def get_dpi()" in text
        assert "def get_scale()" in text

    def test_ios_shim_full_api(self, config, project_root):
        layout = materialize_project(config, project_root)
        text = (layout.platform / "ios.py").read_text(encoding="utf-8")
        for fn in (
            "get_dpi",
            "get_scale",
            "get_density",
            ...
            assert f"def {fn}" in text, f"ios.py missing: def {fn}"

    def test_mobile_shim_written(self, config, project_root):
        layout = materialize_project(config, project_root)
        shim = layout.platform / "mobile.py"
        assert shim.is_file()
        text = shim.read_text(encoding="utf-8")
        for fn in (
            "get_dpi",
            ...
            assert fn in text, f"mobile.py missing: {fn}"

    def test_mobile_shim_ios_delegates_to_ios_module(self, config, project_root):
        layout = materialize_project(config, project_root)
        text = (layout.platform / "mobile.py").read_text(encoding="utf-8")
        assert "from ios import" in text

    def test_mobile_shim_has_desktop_fallbacks(self, config, project_root):
        layout = materialize_project(config, project_root)
        text = (layout.platform / "mobile.py").read_text(encoding="utf-8")
        assert "return 96.0" in text
    ...
    def test_bootstrap_includes_platform_on_pythonpath(self, config, project_root):
        layout = materialize_project(config, project_root)
        bootstrap = (layout.root / "kivy_ios_bootstrap.m").read_text(encoding="utf-8")
        assert "platformPath" in bootstrap
        assert "appPath, platformPath" in bootstrap
    ...
    def test_platform_folder_in_resources(self, config, project_root):
        layout = materialize_project(config, project_root)
        text = (layout.xcodeproj / "project.pbxproj").read_text()
        assert "platform in Resources" in text
