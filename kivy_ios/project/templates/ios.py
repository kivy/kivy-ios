...
place to iterate on the ObjC-runtime wrappers before the interface stabilises
enough to move upstream.

The implementation tracks ``kivy/mobile/_platform/ios.py`` in kivy PR #9331.
...
def get_dpi() -> float:
    ...
    try:
        rt = _objc_runtime()
        device = rt.send_id(rt.get_class(b"UIDevice"), rt.sel(b"currentDevice"))
