...
class ZbarLightRecipe(Recipe):
    ...
    library = "zbarlight.a"
    depends = ['hostpython','python', 'libzbar']
...
#        self.apply_patch("zbarlight_1_2.patch")#Issue getting the version, hard coding for now
    ...
    def install(self):
        ...
        build_dir = join(self.get_build_dir(arch.arch),'build','lib.macosx-10.14-x86_64-2.7','zbarlight')
        dist_dir  = join(self.ctx.dist_dir,'root','python','lib','python2.7','site-packages','zbarlight')
    ...
    def _patch__init__(self):
        init = join(self.ctx.dist_dir,'root','python','lib','python2.7',
