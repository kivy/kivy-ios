...
**`--force`**: without it, init exits non-zero if `[tool.kivy.ios]` already exists. With it, all `[tool.kivy.*]` tables are regenerated **except `[tool.kivy.ios.signing]`**, which is preserved in full. All four signing fields (`team_id`, `identity`, `provisioning_profile`, `auto_signing`) are kept because they are user-specific and project-specific; silently resetting them would break a project that already has working signing configured.
...
| App-local native binaries | project | Scan `app_dir` for `.so`/`.dylib` files; FAIL on any non-iOS-architecture binary (a macOS-compiled extension dropped in `app/` won't load on device — native code belongs in an iOS wheel, see [spec 03 §"App-specific native extensions"](03-artifact-distribution.md)) |
