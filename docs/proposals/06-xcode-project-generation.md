...
- Sync the `app/` and `platform/` folder references in Copy Bundle Resources (as folder references, not group references — preserves the directory structure in the bundle). `pip-deps/` is deliberately **not** a Copy Bundle Resources reference: it is platform-sliced and the Build Python Run Script `rsync`s the correct slice into the bundle (see "The Build Python Run Script"). Any legacy `pip-deps` Copy Bundle Resources reference left by an older toolchain is removed on regeneration. So the installed packages still show in Xcode's navigator, the `pip-deps-simulator` slice is added as a **browse-only** folder reference (displayed as `pip-deps`, with `create_build_files=False` so it belongs to no build phase and has zero build impact); its on-disk path differs from `pip-deps`, so the path-based legacy removal never touches it. The package set is identical across slices, so the simulator slice is a fine stand-in for browsing.
